...
from quasar.config import JSONRPC_EXT_PORT
...
from quasar.dialog import DialogSelect
from quasar.inserttorrent import InsertTorrent
...
class QuasarRPCServer(BaseHandler):
    ...
    def Dialog_Select_Large(self, title, subject, items):
        window = DialogSelect("DialogSelectLarge.xml", ADDON_PATH, "Default", title="%s %s" % (GetLocalizedString(title), subject), items=items)
        window.doModal()
    ...
    def InsertTorrent(self, *args, **kwargs):
        window = InsertTorrent("InsertTorrent.xml", ADDON_PATH, "Default")
        window.doModal()
